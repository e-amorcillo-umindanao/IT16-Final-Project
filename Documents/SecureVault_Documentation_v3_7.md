...
| reCAPTCHA v3 | Google's invisible bot detection service using risk scoring (0.0â€“1.0) |
...
- Google reCAPTCHA v3 bot mitigation on registration and forgot password forms â€” invisible, score-based, fails open if Google is unreachable
...
Google reCAPTCHA v3 is integrated on the Register and Forgot Password forms to mitigate automated bot submissions. The implementation uses the invisible v3 variant â€” no checkbox or image puzzle is presented to the user.
...
The `RECAPTCHA_SITE_KEY` is intentionally shared with the frontend (it is a public key by Google's design). The `RECAPTCHA_SECRET_KEY` is server-side only and must never be exposed in frontend code or version control. If Google's reCAPTCHA service is unreachable, `RecaptchaService` fails open â€” the form submission proceeds and a private error is logged.

**Frontend fail-open sentinel:** If `grecaptcha.execute()` throws (e.g. Google's script fails to load), `lib/recaptcha.ts` returns the sentinel string `__recaptcha_unavailable__` instead of an empty string. The backend `RecaptchaService` recognises this sentinel, logs a private warning, and returns `true` (pass) â€” ensuring the fail-open guarantee is honoured even when the token cannot be obtained client-side. An empty token would otherwise fail Laravel's `nullable` validation rule before reaching the service layer.
